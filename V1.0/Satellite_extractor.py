...
class Sentinel_Tool(object):
    ...
    def login(self):
        api.login(self.username,self.password,save =True)
    ...
    def search(self,csvfile,start,end,dataset=None,):
        ...
        for loc in coordinates:
            response = api.scene_search(dataset,self.node,lat=float(loc[0]),lng=float(loc[1]),start_date=start,end_date=end)#,#extended =True)
            ...
            for r in results:
                ...
                if float(cloud_cover) < 10.0:
                    ids = api.scene_metadata(dataset, entity_id)
                    for id in ids.get('data').get('metadata'):
                        if id.get('fieldName') == 'Vendor Product ID':
                            disp_id = id.get('value')
                    final_candidates.append((disp_id,disp_id1))
